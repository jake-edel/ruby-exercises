RSpec.describe 'map' do
  ...
  it 'squares' do
    numbers = [1, 2, 3, 4, 5]
    squares = numbers.map { |variable| variable * variable }
    expect(squares).to eq([1, 4, 9, 16, 25])
  end

  it 'lengths' do
    names = ["alice", "bob", "charlie", "david", "eve"]
    lengths = names.map{ |variable| variable.length }
    expect(lengths).to eq([5, 3, 7, 5, 3])
  end

  it 'normalize zip codes' do
    numbers = [234, 10, 9119, 38881]
    zip_codes = numbers.map do |variable|
      variable.to_s.rjust(5,'0')
    end
